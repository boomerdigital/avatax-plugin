...
class Main{
    ...
    public function avatax_adminLoadAssets(){
        ...
        wp_enqueue_script( $this->avatax['plugin_prefix'].'admin_js', plugins_url( 'assets/js/admin/admin.js', __FILE__ ), array('jquery'),$this->avatax['version'], true);
        wp_enqueue_script(  $this->avatax['plugin_prefix'].'ajax-script', plugins_url('assets/js/admin/ajax.js', __FILE__ ), array('jquery'),$this->avatax['version'],true );
        wp_enqueue_style(  $this->avatax['plugin_prefix'].'select2css', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css', array(),$this->avatax['version'],'all' );
        wp_enqueue_script( $this->avatax['plugin_prefix'].'select2js', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js', array('jquery'),$this->avatax['version'],true );
    ...
    public function avatax_frontLoadAssets(){
        ...
        wp_enqueue_script( $this->avatax['plugin_prefix'].'front_js', plugins_url( 'assets/js/frontend/frontend.js', __FILE__ ), array('jquery'),$this->avatax['version'], true);
    }

    public function avatax_cssAssets(){
        wp_enqueue_style( $this->avatax['plugin_prefix'].'custom_css', plugins_url( 'assets/css/admin/style.css', __FILE__ ), array(),$this->avatax['version'],'all' );
    }
