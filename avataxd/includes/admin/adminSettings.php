...
class AdminSettings
{
    ...
    public function checkWocommerceExistence()
    {
        if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
            add_action('admin_notices', array($this, 'avatax_adminNotice', 'Wocommerce is must for this Avatax plugin installation. Please install and activate Wocommerce plugin.'));
        }
        add_action('admin_notices', array($this, 'check_wc_version_compatibility'));
    }
    public function check_wc_version_compatibility()
    {
        if (defined('WC_VERSION')) {
            // Define the minimum required WooCommerce version for your plugin
            $required_wc_version = '6.7.1';

            if (version_compare(WC_VERSION, $required_wc_version, '<')) {
                $message = sprintf(
                    __('Avatax plugin requires WooCommerce version %s or later. Please update WooCommerce.', 'avatax'),
                    $required_wc_version
                );
    ...
    public  static function avatax_adminNotice($message)
    {
        global $pagenow;
    ...
    public static function avatax_pluginHtml($settings, $current_section)
    {
        ...
        if ($current_section == 'mysettings') {
            ...
            $settings[] = array(
                'name' => __('Enviroment'), 'type' => 'select',   'id' => 'env', 'options' =>
                array(
            ...
            array(), 'class' => 'calc', 'id' => 'companycode');
